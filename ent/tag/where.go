...
// HasTagItems applies the HasEdge predicate on the "tag_items" edge.
func HasTagItems() predicate.Tag {
	return predicate.Tag(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(TagItemsTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, TagItemsTable, TagItemsPrimaryKey...),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasTagItemsWith applies the HasEdge predicate on the "tag_items" edge with a given conditions (other predicates).
func HasTagItemsWith(preds ...predicate.Item) predicate.Tag {
	return predicate.Tag(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(TagItemsInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, TagItemsTable, TagItemsPrimaryKey...),
		)
