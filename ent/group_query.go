...
func (gq *GroupQuery) QueryGroupItems() *ItemQuery {
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withGroupItems: gq.withGroupItems.Clone(),
...
func (gq *GroupQuery) WithGroupItems(opts ...func(*ItemQuery)) *GroupQuery {
	query := &ItemQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withGroupItems = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context) ([]*Group, error) {
	...
	if query := gq.withGroupItems; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Group, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.GroupItems = []*Item{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(group.GroupItemsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, gq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "group_items": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "group_items" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.GroupItems = append(nodes[i].Edges.GroupItems, n)
			}
		}
	}
