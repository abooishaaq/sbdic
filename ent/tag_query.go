...
// QueryTagItems chains the current query on the "tag_items" edge.
func (tq *TagQuery) QueryTagItems() *ItemQuery {
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withTagItems: tq.withTagItems.Clone(),
...
// WithTagItems tells the query-builder to eager-load the nodes that are connected to
// the "tag_items" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TagQuery) WithTagItems(opts ...func(*ItemQuery)) *TagQuery {
	query := &ItemQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withTagItems = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context) ([]*Tag, error) {
	...
	if query := tq.withTagItems; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Tag, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.TagItems = []*Item{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(tag.TagItemsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, tq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "tag_items": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "tag_items" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.TagItems = append(nodes[i].Edges.TagItems, n)
			}
		}
	}
