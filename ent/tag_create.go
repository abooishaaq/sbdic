...
func (tc *TagCreate) AddTagItemIDs(ids ...int) *TagCreate {
	tc.mutation.AddTagItemIDs(ids...)
	return tc
}

// AddTagItems adds the "tag_items" edges to the Item entity.
func (tc *TagCreate) AddTagItems(i ...*Item) *TagCreate {
	ids := make([]int, len(i))
	for j := range i {
		ids[j] = i[j].ID
	}
	return tc.AddTagItemIDs(ids...)
}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.TagItemsIDs(); len(nodes) > 0 {
