...
func (iq *ItemQuery) QueryTag() *TagQuery {
...
func (iq *ItemQuery) WithTag(opts ...func(*TagQuery)) *ItemQuery {
	query := &TagQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *ItemQuery) sqlAll(ctx context.Context) ([]*Item, error) {
	...
	if query := iq.withGroup; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(item.GroupPrimaryKey[1], fks...))
			},
	...
	if query := iq.withTag; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(item.TagPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
